...
class BatchContext {
  ...
  constructor (incomingODataRequest, incomingODataResponse) {
    ...
    /**
     * Store the first framework error, framework errors lead to a complete batch fail.
     * Some framework errors from exits (e.g. from GROUP_START or GROUP_END exit) are not stored here
     * because this framework errors just lead to an failure of the specific atomicity group.
     * @type {Error}
     * @private
     */
    this._frameworkError = null
  }

  /**
   * Set framework error
   * @param {Error} error Framework error
   */
  setFrameworkError (error) {
    this._frameworkError = error
  }

  /**
   * Get framework error
   * @returns {Error} Framework error
   */
  getFrameworkError () {
    return this._frameworkError
  }
   ...
   * @param {Error} [applicationError] Error to be forwarded to markAtomicityGroupAsFailed as root cause for the failed atomicity group
   * @returns {BatchContext} this instance
   */
  markRequestAsFailed (id, request, response, applicationError) {
    const atomicityGroupId = request.getAtomicityGroupId()

    if (this._failedRequestsPerAtomicityGroup.has(atomicityGroupId)) {
      const erroneousRequests = this._failedRequestsPerAtomicityGroup.get(atomicityGroupId)
      // don't add request/response twice to the failed requests per atomicity group array.
      let found = erroneousRequests.some(entry => entry.getId() === id)
      if (!found) {
        const batchErrorInfo = new BatchErrorInfo(id, request, response)
        erroneousRequests.push(batchErrorInfo)
      }
    } else {
      const batchErrorInfo = new BatchErrorInfo(id, request, response)
    ...
    if (!this._failedAtomicityGroups.get(atomicityGroupId)) {
      this.markAtomicityGroupAsFailed(atomicityGroupId, applicationError)
    }
    return this
  }
