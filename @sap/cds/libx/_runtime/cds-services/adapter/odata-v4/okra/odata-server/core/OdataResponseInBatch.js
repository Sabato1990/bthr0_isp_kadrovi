...
class OdataResponseInBatch {
  ...
  /**
   * Sets the response object which is given to the service processor to store the response in memory
   * Later the response objects data is used to build the multipart/json batch response
   * @param {Object} inResponse response object
   * @returns {OdataResponseInBatch} this instance
   */
  setInResponse (inResponse) {
    this._inResponse = inResponse
    return this
  }
  /**
