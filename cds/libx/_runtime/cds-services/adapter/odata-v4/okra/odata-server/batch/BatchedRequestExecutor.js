...
const HeaderNames = require('../../odata-commons').http.HttpHeader.HeaderNames
...
class BatchedRequestExecutor {
  ...
  execute () {
    ...
    try {
      return service.process(plainHttpRequest, plainHttpResponse).then((/* ignore result, read status code */) => {
